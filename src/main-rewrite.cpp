...
auto reqForGhAPI() {
    ...
    req.header(AuthorizationHeaderName(), fmt::format("Bearer {}", get_ghc()));
    return req;
}
...
#if 1
class ModListSource;
class ModList : public CCNode {}; class ModsStatusNode : public CCNode {};
    ...
    static IssueItem* create(matjson::Value pJson, CCContentLayer* pContentLayer = nullptr, ScrollLayer* pScrollLayer = nullptr) {
        ...
        if (pRtn->init()) {
            ...
            //right shit
            {
                ...
                /*ViewBtn*/ {
                    //text
                    CCLabelBMFont* View = CCLabelBMFont::create("   View   ", "Comic Sans MS.fnt"_spr);
                    ...
                    auto padding = 5.f;
                    grad->setContentSize(ViewBtn->getContentSize() + CCPoint(padding, padding));
                    grad->setPosition(CCPoint(-padding, -padding) / 2);
                    ViewBtn->addChild(grad, -1, 57290);
                }
                //id
...
public:
    ...
    void setupList() {
        ...
        for (const auto& entry : fs::directory_iterator(ryzen_dir / "mods")) {
            ...
            auto main_jsonp = entry.path() / "main.json";
...
public:
    ...
    void downloadModsFinish(web::WebResponse* res) {
        auto loading_circle = dynamic_cast<LoadingCircle*>(this->getChildByIDRecursive("loading_circle"));
        if (loading_circle) loading_circle->fadeAndRemove();
        //resp
        std::string data = res->string().unwrapOr("");
        //json
        std::string error;
        auto json_val = matjson::parse(data, error);
        if (error.size() > 0) data = ("Error parsing JSON: " + error + " in :" + data);
        //a
        if (res->code() > 399) {
            loading_circle->fadeAndRemove();
            auto message = data;
            auto json = matjson::parse(message, error);
            ...
            fs::create_directories(local_issues_save.parent_path());
            std::ofstream(local_issues_save.string().c_str()) << json_val.value().dump();
            loading_circle->fadeAndRemove();
        }
    }
    void downloadMods() {
        ...
        m_DownloadListTaskListener;
        auto req = reqForGhAPI();
        auto& listener = m_DownloadListTaskListener;
        listener.bind(
            [this, &loading_circle](web::WebTask::Event* e) {
                if (web::WebResponse* res = e->getValue()) downloadModsFinish(res);
            }
        );
        listener.setFilter(req.send("GET", urlapi));
    }
